...
type Channel struct {
	...
	ErrorResponseMapping *string `json:"error_response_mapping" gorm:"type:text"`
...
func BatchDeleteChannels(ids []int) (int64, error) {
	...
	for _, chunk := range lo.Chunk(ids, 200) {
		...
		if err := tx.Where("channel_id in (?)", chunk).Delete(&ChannelModelState{}).Error; err != nil {
			tx.Rollback()
			return 0, err
		}
	}
...
func (channel *Channel) GetErrorResponseMapping() string {
	if channel.ErrorResponseMapping == nil {
		return ""
	}
	return *channel.ErrorResponseMapping
}
...
func (channel *Channel) Delete() error {
	...
	return DB.Where("channel_id = ?", channel.Id).Delete(&ChannelModelState{}).Error
}
...
func DeleteDisabledChannel() ([]int, int64, error) {
	var ids []int
	...
	rows, err := BatchDeleteChannels(ids)
	return ids, rows, err
}
