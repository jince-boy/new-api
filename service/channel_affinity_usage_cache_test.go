...
func channelAffinityUsageCacheTestIDs(t *testing.T) (string, string, string) {
	t.Helper()
	suffix := strings.NewReplacer("/", "_", " ", "_").Replace(t.Name())
	unique := fmt.Sprintf("%s_%d_%d", suffix, time.Now().UnixNano(), channelAffinityUsageCacheTestSeq.Add(1))
	return "rule_" + unique, "default", "fp_" + unique
}
...
func TestObserveChannelAffinityUsageCacheByRelayFormat_ClaudeMode(t *testing.T) {
	ruleName, usingGroup, keyFP := channelAffinityUsageCacheTestIDs(t)
