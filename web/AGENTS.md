...
- 客户接口文档统一展示在模型广场详情页的 API Tab，不再维护独立的 `/docs` 页面或导航入口。
- 独立 API 接口文档与外部“使用文档 / User Guide”是两个功能；必须保留使用文档导航、`general_setting.docs_link` 及其可见性和打开方式配置，除非产品需求明确要求移除使用文档。
- API Tab 必须以定价接口返回的模型级 `supported_endpoint_types` 与 `supported_endpoints` 为事实来源；模型显式配置的路径、HTTP 方法和端点类型优先于渠道级推断。
- 每个端点必须展示对应协议的鉴权方式、内容类型、必填/可选参数、请求示例、真实响应示例，以及必要的流式、重试或异步轮询说明。不得用模型名称猜测接口能力，也不得展示随机生成的速率限制或参数。
- 新增、删除或调整公共模型接口时，必须在同一变更中更新 API Tab 使用的端点目录数据、根目录 `docs/model-api.md` 与 `docs/openapi/relay.json`。方法、路径、鉴权、请求字段、响应结构、流式协议、任务状态、限制或错误语义发生变化时同样必须更新。
- 新增文案必须通过项目 i18n 流程补齐 `en`、`zh`、`zh-TW`、`fr`、`ja`、`ru`、`vi`，不得只提供单一语言。
- 端点选择、模型级路径覆盖或能力过滤发生变化时，必须更新 `src/features/pricing/__tests__/` 中的覆盖测试，并核对端点切换、代码复制与响应式布局。
...
- **2026-07-22**：新增客户接口文档维护规范，要求公共接口变更同步更新 `/docs`、Markdown、OpenAPI 与多语言内容。
- **2026-07-29**：将客户接口文档收拢到模型广场详情页 API Tab，移除独立 `/docs` 页面及其导航配置。
