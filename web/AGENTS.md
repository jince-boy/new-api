...
- 客户接口文档页面位于 `/docs`，实现与目录数据位于 `src/features/api-docs/`，公共路由定义位于 `src/routes/docs/`。
- 新增、删除或调整公共模型接口时，必须在同一变更中更新 `src/features/api-docs/data/catalog.ts`。方法、路径、鉴权、请求字段、响应结构、流式协议、任务状态、限制或错误语义发生变化时同样必须更新。
- 每个面向客户的接口条目必须提供易于理解的用途说明、必填/可选参数、真实响应示例，以及必要的限制、重试或异步轮询说明；页面保持以接口参考文档为核心，除非产品需求明确要求，否则不要额外增加独立的请求代码示例。避免内部实现术语和模板化、机械化文案。
- 同步维护根目录 `docs/model-api.md` 与 `docs/openapi/relay.json`。页面目录、Markdown 与 OpenAPI 应描述同一份公开契约。
- 新增文案必须通过项目 i18n 流程补齐 `en`、`zh`、`zh-TW`、`fr`、`ja`、`ru`、`vi`，不得只提供单一语言。
- 接口目录或文档导航发生变化时，必须更新 `src/features/api-docs/__tests__/` 中的覆盖测试，并核对移动端导航、搜索、深链接和代码复制行为。
