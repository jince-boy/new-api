...
import { resetModelRatios, updateSystemGroupOptions } from '../api'
...
const createModelSchema = (t: Translate) =>
  z.object({
    ...
    PerSecondRules: createJsonStringField(t),
  })
...
export function RatioSettingsCard({
  ...
  const groupOptionsMutation = useMutation({
    mutationFn: async (options: Record<string, string | boolean | number>) => {
      const data = await updateSystemGroupOptions({ options })
      if (!data.success) {
  ...
  const modelNormalizedDefaults = useRef({
    ...
    PerSecondRules: normalizeJsonString(modelDefaults.PerSecondRules),
  })
  ...
  const modelForm = useForm<ModelFormValues>({
    ...
    defaultValues: {
      ...
      PerSecondRules: formatJsonForTextarea(modelDefaults.PerSecondRules),
    },
  })
  ...
  const saveModelRatios = useCallback(
    async (values: ModelFormValues) => {
      const normalized = {
        ...
        PerSecondRules: normalizeJsonString(values.PerSecondRules),
      }

      const apiKeyMap: Record<string, string> = {
        BillingMode: 'billing_setting.billing_mode',
        BillingExpr: 'billing_setting.billing_expr',
        PerSecondRules: 'billing_setting.per_second_rules',
      }
  ...
  const saveGroupRatios = useCallback(
    async (values: GroupFormValues) => {
      ...
      for (const key of updates) {
        ...
        options[apiKey] = normalized[key]
      }

      if (updates.length === 0) return
      await groupOptionsMutation.mutateAsync(options)

      groupNormalizedDefaults.current = normalized
    },
    [groupOptionsMutation]
  )
    ...
    if (tab === 'groups') {
      return (
        <GroupRatioForm
          ...
          isSaving={groupOptionsMutation.isPending}
        />
      )
    }
    if (tab === 'tool-prices') {
