...
- 安全模型的 URL、API Key、模型名称、超时、上下文长度和并发数均可在管理后台配置，默认实现以 Qwen3Guard 的返回格式为准。
- 针对模型 32K 上下文限制，系统会对上下文进行有界分片并发审查，控制单次内存和并发规模；审查耗时不计入用户请求的首字响应时间。
- 可按渠道选择是否启用智能保护。命中配置的 `Safety` 或 `Categories` 时拦截请求并退还预扣额度；安全模型不可用或超时时默认放行，不阻断正常请求。
- 每次拦截都会保存保护事件，包括用户、渠道、请求 ID、模型、命中结果、风险内容、原始审查结果和审查耗时，并支持服务端分页和详情查看；长内容在详情页以限高滚动区域展示。
