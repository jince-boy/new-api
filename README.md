...
- 拦截条件使用规则组配置：每条规则可选择“满足任意条件”或“满足全部条件”，不同规则之间使用或逻辑；因此既可以拦截所有 `Safety=Controversial`，也可以精确配置为 `Controversial + Jailbreak`。Qwen3Guard 的“非暴力违法行为”通常会返回 `Safety=Unsafe`，如果要拦截它，应选择 `Unsafe` 或把该 Category 加入“满足任意条件”的规则。
