...
- 支持按渠道配置独立的请求速率窗口：最大请求数（`max_requests`）和窗口时长（`window_seconds`）。未配置或配置不完整时不会启用渠道限流。
- 请求转发到上游前先进行原子预约，避免并发请求超卖；达到限制时会尝试其他可用渠道，并可在有限等待范围内等待最早恢复的渠道。
- 如果请求最终没有真正发起上游调用，会释放已预约的额度，避免无效占用。
- 普通中转和异步任务路径均支持渠道级限流，渠道编辑页面提供可视化配置。
- 当所有可用渠道都达到限制时，返回明确的渠道限流错误，不影响其他未触发限制的渠道继续服务。

### 3. 智能保护

- 在配额预扣完成后、实际调用上游模型前，使用 OpenAI 兼容的安全模型审查请求上下文；当前按 `Qwen3Guard-Gen-4B` 的 `Safety` 和 `Categories` 结果进行判断。
- 拦截条件使用有序规则组配置：每次安全模型审查结果从上到下只采用第一条命中的规则，后续规则不会叠加动作；同一请求被分片或扩展上下文多次审查时，每次审查结果分别执行自己的规则动作。每条规则可选择“满足任意条件”或“满足全部条件”（作用于 Categories，Safety 始终是等级门槛），因此既可以拦截所有 `Safety=Controversial`，也可以精确配置为 `Controversial + Jailbreak`。Qwen3Guard 的“非暴力违法行为”通常会返回 `Safety=Unsafe`，如果要拦截它，应选择 `Unsafe` 或把该 Category 加入规则。
- 安全模型的 URL、API Key、模型名称、超时、上下文长度和并发数均可在管理后台配置；URL、API Key 和模型名称初始为空，由管理员自行填写，当前解析逻辑以 Qwen3Guard 的返回格式为准。
- 采用分级审查策略：每次请求优先审查最新用户消息并附带最近几轮上下文；只有消息引用历史、会话近期出现风险或最新消息过长时，才扩大到更早的上下文，避免长对话重复审核全部历史。
- 针对模型 32K 上下文限制，系统会对超长最新消息进行有界分片并发审查，控制单次内存和并发规模；审查耗时不计入用户请求的首字响应时间和总耗时，并在管理员日志中单独展示。
- 对相同安全模型配置和相同文本分片复用短期分类结果缓存，并设置 10 分钟过期时间、条目数量和总大小上限；缓存只保存分类结果，不保存用户原文，过期条目会被清理，避免内存无限增长。
- 可按渠道选择是否启用智能保护。命中配置的 `Safety` 或 `Categories` 时拦截请求并退还预扣额度；安全模型不可用或超时时默认放行，不阻断正常请求。
- 每次拦截都会保存保护事件，包括用户、渠道、请求 ID、模型、命中结果、风险内容、原始审查结果和审查耗时，并支持服务端分页和详情查看；长内容在详情页以固定高度的局部滚动区域展示，避免撑破页面。
- 管理员可以在近期保护事件页经过二次确认后一键清除全部保护事件；事件还会按保留天数自动清理。
- `Safety` 与 `Categories` 结果在管理页面支持中文展示。用户存在邮箱且开启邮件告警时，每次拦截都会发送警告邮件；邮箱为空时不发送。邮件会明确说明请求已被智能保护拦截，若管理员确认存在越狱、破限等操作，对应账号将直接冻结；邮件发送失败会记录到事件和服务端日志。
- 管理入口位于 `/smart-protection`，与智能调度同级；页面默认先显示“近期保护事件”，再显示“保护配置”。旧地址 `/system-settings/security/smart-protection` 会自动跳转到新页面。

相关接口：

- `GET /api/smart-protection/settings`
- `PUT /api/smart-protection/settings`
- `GET /api/smart-protection/channels`
- `GET /api/smart-protection/events`
- `GET /api/smart-protection/events/:id`
...
- 需要补税或补差时，可由管理员配置通过易支付完成补充支付，或在余额充足时直接扣除用户余额。
